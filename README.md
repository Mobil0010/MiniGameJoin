<div align="center">
  ...
  <p>
    ...
    현재 Yacht Dice의 로컬·온라인 2인 플레이와<br />
    AWS 기반 가위바위보 온라인 2~6인 플레이와<br />
    스피드 퀴즈 온라인 4~10인 팀 대항 플레이를 제공합니다.
  </p>
...
>
> Yacht Dice, 가위바위보, 스피드 퀴즈의 회원/게스트 인증, 온라인 방, 채팅과 서버 권위형 게임 처리를 실제 AWS 환경에서 제공합니다.
...
현재 **Yacht Dice**, **가위바위보**, **스피드 퀴즈**를 구현했습니다. Yacht Dice는 한 화면에서 두 명이 번갈아 플레이하는 로컬 모드와 초대 코드 기반 온라인 모드를 지원합니다. 가위바위보는 2~6명이 참가하는 실시간 온라인 모드로 제공하며, 1:1 토너먼트와 전체 난투전을 지원합니다. 스피드 퀴즈는 4~10명이 두 팀으로 나뉘어 제한시간 동안 제시어를 설명하고 맞히는 온라인 팀 대항 릴레이입니다.
...
- 가위바위보 선택과 승패, 토너먼트 대진을 서버에서 판정하는 서버 권위형 진행
- 스피드 퀴즈 제시어 추첨, 정답 비교, 점수와 턴 전환을 서버에서 검증
- 새 게임이 추가될 때마다 승패와 승률을 별도로 누적하는 게임별 전적 구조
...
- 가위바위보 손 선택, 카운트다운, 공개, 무승부, 라운드·최종 승패 효과음
...
- Yacht Dice, 가위바위보, 스피드 퀴즈 공개방 생성, 게임별 공개방 목록과 즉시 참가
...
- Yacht Dice와 가위바위보 전적을 게임별로 분리해 조회
- 회원 검색, 친구 요청·수락·삭제, 접속 상태와 방 초대

### 가위바위보 온라인 모드

- 최소 2명, 최대 6명이 초대 코드로 같은 방에 참가
- 별도 대기 로비 없이 게임 화면에서 바로 참가자 대기 및 게임 시작
- `1:1 토너먼트`와 `전체 난투전` 방식
- 방장이 선택 제한시간, 승리 조건·생명, 방 정원을 설정
- 모바일 오른쪽 위 `규칙 설정` 버튼과 설정 모달
- 모바일 상단에 게임 시작, 준비, 방 나가기와 사운드 버튼 배치
- 준비 전에는 `준비 하기`, 준비 완료 후에는 `대기 중`으로 표시하고 다시 눌러 준비 취소
- 선택 제한시간이 끝나면 미선택 플레이어의 손을 서버에서 무작위 결정
- 각 플레이어의 선택은 결과 공개 전까지 비공개 서버 필드에 저장
- 결과 공개 후 자동 진행하지 않고 방장이 `다음 턴`을 눌러야 다음 라운드·대진 진행
- 다음 턴 권한을 클라이언트와 Lambda 양쪽에서 방장으로 제한
- 남은 시간에 맞춰 줄어드는 원형 카운트다운 게이지
- 대기 중 뒤로가기를 누르면 방 나가기 확인
- 탭·창·앱 종료 감지 시 대기 중에는 방 나가기, 경기 중에는 기권 처리
- 대기·선택·긴급 카운트다운·승패 상황에 따라 다른 배경음악과 효과음

### 스피드 퀴즈 온라인 모드

- 최소 4명, 최대 10명이 같은 방에 참가
- 입장 순서에 따라 A팀과 B팀으로 균형 배정
- 팀마다 세 차례씩 번갈아 진행하는 총 6차례 릴레이
- 차례마다 60초 제한시간과 최대 3회 패스
- 중복 없는 1,024개 제시어를 Lambda에서 무작위 추첨
- 한 경기에서 이미 나온 제시어는 다시 나오지 않으며 전체 목록 소진 후 초기화
- 현재 설명자에게만 제시어를 제공하는 전용 GraphQL Query
- 같은 팀의 맞히는 플레이어에게만 정답 입력창 표시
- 정답은 Lambda가 제시어와 비교하고 일치하면 즉시 1점과 다음 문제 반영
- 정답 비교 시 띄어쓰기와 영문 대소문자 차이 허용
- 설명자가 음성 답변을 직접 판정할 수 있는 `정답 +1`과 `패스` 버튼
- 상대 팀과 설명자의 정답 입력을 클라이언트와 Lambda에서 모두 차단
- 팀별 누적 점수, 현재 설명자, 남은 시간과 최종 승리·무승부 표시
- 경기 종료 후 같은 방에서 다시 시작 가능

## 가위바위보 규칙

### 1:1 토너먼트

참가자를 무작위 대진으로 배치해 두 명씩 대결합니다. 방장이 지정한 1~3승 선취 조건을 먼저 달성한 플레이어가 다음 대진으로 진출하며, 마지막까지 남은 플레이어가 최종 승리합니다. 홀수 인원에서는 한 명이 다음 단계로 자동 진출할 수 있습니다.

### 전체 난투전

현재 생존한 모든 플레이어가 동시에 손을 선택합니다. 가위·바위·보가 모두 나오거나 모두 같은 손을 내면 무승부이며, 승패가 결정되면 패배한 플레이어의 생명이 하나 줄어듭니다. 마지막 한 명이 남을 때까지 진행합니다.

### 제한시간과 턴 진행

- 선택 제한시간은 5초, 10초, 15초, 20초 중에서 방장이 지정합니다.
- 시간 안에 선택하지 않으면 Lambda가 암호학적 난수로 가위·바위·보 중 하나를 결정합니다.
- 모든 선택이 끝나면 결과는 자동으로 공개되지만 다음 턴은 자동으로 시작되지 않습니다.
- 방장이 결과를 확인한 뒤 `다음 턴`을 눌러야 다음 라운드 또는 다음 대진이 시작됩니다.

## 스피드 퀴즈 규칙

참가자 4~10명은 A팀과 B팀으로 자동 배정됩니다. A팀과 B팀이 번갈아 한 차례씩 진행하며 각 팀은 세 차례, 전체 경기는 총 여섯 차례로 구성됩니다. 같은 팀의 설명자는 차례마다 순서대로 교대합니다.

설명자는 자신의 화면에만 표시되는 제시어를 보고 같은 팀원에게 말로 설명합니다. 제시어 자체나 제시어에 포함된 글자를 직접 말하면 안 됩니다. 같은 팀원은 정답창에 단어를 입력하며, 서버에서 현재 제시어와 일치하는지 확인한 뒤 정답이면 즉시 1점이 추가되고 다음 제시어로 넘어갑니다.

- 한 차례의 제한시간은 60초입니다.
- 패스는 차례당 최대 3회 사용할 수 있습니다.
- 정답 입력의 앞뒤·중간 띄어쓰기와 영문 대소문자는 무시합니다.
- 설명자는 말로 나온 정답을 `정답 +1` 버튼으로 직접 처리할 수도 있습니다.
- 제한시간이 끝나면 다음 팀과 다음 설명자로 전환됩니다.
- 여섯 차례의 합산 점수가 높은 팀이 승리하며 점수가 같으면 무승부입니다.
- 1,024개 제시어는 서버에서 추첨하고 현재 설명자 외 참가자에게는 공개하지 않습니다.
...
flowchart LR
    ...
    Lambda --> GameStats["DynamoDB GameStats"]
    Lambda --> Chat["DynamoDB ChatMessages"]
    Lambda --> Friends["DynamoDB Friends"]
...
| AWS Lambda | 주사위·가위바위보·스피드 퀴즈 규칙, 권한, 점수, 방 상태, 승패 처리 |
| Amazon DynamoDB | 회원, 방, 경기, 게임별 전적, 채팅, 친구 데이터 |
...
- `OnlineRpsGame.tsx`: 가위바위보 대기, 규칙 설정, 손 선택, 결과와 수동 턴 진행
- `OnlineSpeedQuizGame.tsx`: 스피드 퀴즈 설명자·정답자 화면, 타이머, 점수와 팀 릴레이 진행
- `OnlineChatPanel.tsx`: 채팅 내역과 입력 UI
- `FriendsPanel.tsx`: 친구 검색, 요청, 접속 상태와 방 초대
- `MatchHistoryDialog.tsx`: 전적 목록과 상세 점수
- `MemberProfileDialog.tsx`: 닉네임과 이메일 변경
- `audio/yachtAudio.ts`: Yacht Dice 상황별 배경음악 제어
- `audio/rpsAudio.ts`: 가위바위보 상황별 음악과 효과음 제어
...
온라인 게임에서는 클라이언트가 최종 주사위 값·점수·가위바위보 결과·스피드 퀴즈 정답을 임의로 확정하지 않습니다.
...
- 가위바위보 선택은 결과 공개 전까지 비공개 방 필드에 저장
- 제한시간이 끝난 미선택 손은 Lambda에서 무작위 결정
- 토너먼트 대진, 전체 난투전 생명, 라운드 승패를 Lambda에서 계산
- 결과 공개 후 다음 턴 진행은 방장 요청만 허용
- 스피드 퀴즈 제시어는 Lambda의 암호학적 난수로 추첨하고 비공개 방 필드에 저장
- 현재 설명자만 제시어 Query를 호출할 수 있으며 다른 참가자의 요청은 거부
- 현재 팀의 정답자만 답안을 제출할 수 있고 Lambda가 정규화한 제시어와 직접 비교
- 정답, 패스, 제한시간 종료와 다음 설명자 전환을 방 버전과 함께 조건부 저장
...
- 주사위 굴림 버튼 연속 클릭
- 동시에 같은 점수 항목 확정
- 동시에 여러 가위바위보 손 선택 또는 다음 턴 요청
- 여러 팀원이 같은 스피드 퀴즈 정답을 동시에 제출하는 상황
- 준비 상태와 게임 시작 요청 충돌
- 기권과 정상 게임 종료의 중복 기록
...
- 진행 중 게임은 게임 화면에서 주기적으로 heartbeat 전송
...
- 가위바위보 대기 상태의 뒤로가기는 방 나가기 여부를 확인
- 가위바위보 결과 화면은 방장이 `다음 턴`을 누르기 전까지 유지
...
| `gameId` | Yacht Dice, 가위바위보 또는 스피드 퀴즈 게임 구분 |
| `players` | 게임별 최대 10명의 ID, 닉네임, 방장/준비 상태, 플레이어·관전자 역할과 점수 |
| `activePlayerId` | 현재 차례 플레이어 |
| `dice` | 서버에서 생성한 주사위 값과 보관 상태 |
| `rollCount` | 현재 턴의 굴림 횟수 |
| `rpsSettings` | 게임 방식, 제한시간, 승리 조건·생명, 최대 인원 |
| `rpsPhase` | 손 선택 또는 결과 공개 상태 |
| `rpsCurrentPlayerIds` | 현재 대결에 참가하는 플레이어 |
| `rpsPlayerStates` | 플레이어별 승수, 생명, 탈락 상태 |
| `speedQuizPhase` | 스피드 퀴즈 차례 대기 또는 문제 진행 상태 |
| `speedQuizTurn` / `speedQuizTotalTurns` | 현재 차례와 전체 차례 수 |
| `speedQuizActiveTeam` / `speedQuizDescriberId` | 현재 팀과 설명자 |
| `speedQuizTeamAScore` / `speedQuizTeamBScore` | 팀별 누적 점수 |
| `speedQuizPassCount` / `speedQuizTurnDeadline` | 현재 차례의 패스 횟수와 종료 시각 |
| `_speedQuizWord` / `_speedQuizUsedWords` | 외부 GraphQL 타입에 노출하지 않는 현재 제시어와 사용 목록 |
...
### `MiniGameJoinGameStats`

회원의 누적 전적을 게임별로 분리해 저장합니다.

- Partition Key: `userId`
- Sort Key: `gameId`
- Yacht Dice와 가위바위보의 승리, 패배, 무승부, 승률은 서로 영향을 주지 않음
- 앞으로 게임을 추가해도 같은 키 구조로 독립 전적을 저장

### `MiniGameJoinFriends`

회원 간 친구 관계, 받은 친구 요청, 최근 접속 상태와 방 초대 코드를 저장합니다. 친구 기능은 회원에게만 제공하며 게스트는 접근할 수 없습니다.
...
| `myMatchHistory` | 회원 | 게임별 내 전적 페이지 조회 |
| `matchDetail` | 회원 | 경기별 전체 점수표 조회 |
| `speedQuizPrompt` | 회원/게스트 | 현재 설명자에게만 스피드 퀴즈 제시어 제공 |
...
| `selectPlayers` | 회원/게스트 | Yacht Dice 플레이어 선택 |
| `updateRpsSettings` | 회원/게스트 | 방장이 가위바위보 규칙 변경 |
| `rollDice` | 회원/게스트 | 서버 주사위 굴림 |
| `confirmScore` | 회원/게스트 | 서버 점수 확정 |
| `submitRpsHand` | 회원/게스트 | 비공개 가위바위보 손 제출 |
| `advanceRpsRound` | 회원/게스트 | 제한시간 결과 공개 또는 방장의 다음 턴 진행 |
| `startSpeedQuizTurn` | 회원/게스트 | 현재 설명자가 60초 차례 시작 |
| `scoreSpeedQuizPrompt` | 회원/게스트 | 설명자의 정답·패스 처리 또는 제한시간 종료 |
| `submitSpeedQuizAnswer` | 회원/게스트 | 현재 팀원이 입력한 정답을 서버에서 비교 |
| `returnToWaitingRoom` | 회원/게스트 | 종료된 방을 다시 대기 상태로 변경 |
| `forfeit` | 회원/게스트 | 기권 처리 |
| `heartbeat` | 회원/게스트 | 접속 상태 갱신 |
| `claimDisconnectWin` | 회원/게스트 | 연결 종료 승리 확인 |
| `sendChatMessage` | 회원/게스트 | 채팅 전송 |
| `touchPresence` | 회원 | 친구 목록용 접속 상태 갱신 |
| `sendFriendRequest` | 회원 | 친구 요청 전송 |
| `respondFriendRequest` | 회원 | 친구 요청 수락 또는 거절 |
| `removeFriend` | 회원 | 친구 삭제 |
| `inviteFriendToRoom` | 회원 | 친구에게 현재 방 초대 |

### Subscription

| 필드 | 설명 |
|---|---|
| `onRoomChanged` | 방 참가, 준비, 시작, 주사위, 가위바위보, 스피드 퀴즈, 기권 상태 변경 |
...
- 가위바위보 손은 공개 전까지 비공개 필드에 저장하고 승패는 Lambda에서 판정합니다.
- 스피드 퀴즈 제시어는 Room GraphQL 타입에 포함하지 않고 현재 설명자 전용 Query로만 제공합니다.
- 스피드 퀴즈 답안은 현재 팀원만 제출할 수 있으며 정답 문자열은 Lambda 안에서 비교합니다.
- 방장만 규칙 변경과 결과 공개 후 다음 턴 진행을 요청할 수 있습니다.
- 회원의 활성 방 점유를 조건부로 기록해 여러 로그인 세션에서 중복 생성·참가하지 못하게 합니다.
...
- 가위바위보 설정값 정규화와 2인 승패 조합
- 전체 난투전의 무승부·다중 승자·패자 판정
- 토너먼트 대진 진행과 홀수 인원 부전승 처리
- 스피드 퀴즈 제시어 1,024개와 전체 중복 검사
- 스피드 퀴즈 정답의 띄어쓰기·영문 대소문자 정규화
...
- [스피드 퀴즈 AppSync/Lambda 배포](./backend/appsync/SPEED_QUIZ_DEPLOY.md)
...
- 가위바위보 다중 기기 동기화와 네트워크 복구 검증 강화
- 스피드 퀴즈 카테고리 선택과 방장 규칙 설정
