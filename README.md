<div align="center">
  ...
  <p>
    ...
    AWS 기반 가위바위보 온라인 2~6인 플레이를 제공합니다.
  </p>
...
>
> Yacht Dice와 가위바위보의 회원/게스트 인증, 온라인 방, 채팅, 게임별 전적, 관전 및 기권 처리를 실제 AWS 환경에서 제공합니다.
...
현재 **Yacht Dice**와 **가위바위보**를 구현했습니다. Yacht Dice는 한 화면에서 두 명이 번갈아 플레이하는 로컬 모드와 초대 코드 기반 온라인 모드를 지원합니다. 가위바위보는 2~6명이 참가하는 실시간 온라인 모드로 제공하며, 1:1 토너먼트와 전체 난투전을 지원합니다.
...
| TypeScript 6 | Yacht Dice·가위바위보 상태, API DTO, 사용자/방 모델의 정적 타입 검사 |
...
| AWS Lambda | 주사위·가위바위보 규칙, 권한, 점수, 방 상태, 승패 처리 |
...
온라인 게임에서는 클라이언트가 최종 주사위 값·점수·가위바위보 결과를 임의로 제출하지 않습니다.
...
| `gameId` | Yacht Dice 또는 가위바위보 게임 구분 |
| `players` | 최대 6명의 ID, 닉네임, 방장/준비 상태, 플레이어·관전자 역할과 점수 |
...
| 필드 | 설명 |
|---|---|
| `onRoomChanged` | 방 참가, 준비, 시작, 주사위, 가위바위보, 기권 상태 변경 |
